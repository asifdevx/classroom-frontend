  ...
  if (query.isLoading || query.isError || !details) {
    return (
      <ShowView className="class-view">
        ...
        <p className="text-sm text-muted-foreground">{query.isLoading ? "Loading subject details..." : query.isError ? "Failed to load subject details." : "Subject details not found."}</p>
      </ShowView>
    );
  }
      ...
      <Card className="hover:shadow-md transition-shadow">
        ...
        <CardContent className="space-y-2">
          {details.subject.department ? (
            <>
              <Link to={`/departments/show/${details.subject.department.id}`} className="text-lg font-semibold text-foreground hover:underline">
                {details.subject.department.name}
              </Link>
              <p className="text-sm text-muted-foreground">{details.subject.department.description ?? "No department description provided."}</p>
            </>
          ) : (
